...
public class TicTacToe {
    ...
    boolean gameOver = false; // set game over to true in gameStatues
    ...
    */
    public void play(){
        while (!gameOver) {  // Loop until the game ends
            ...
            switch (gameStat) {
                ...
                case WIN:
                    if (!firstPlayer) { // Set winner from gameStatues
    ...
    /* printStatus(player, int)
        Going to be printing out whose turn it is, who won or if it's a tie
        void, because it's not going to return anything, just outputting the standing for each player
        Also going to ask what position for their move
            going to run validMove()
     */
    public void printStatus(int player) {
        Scanner scanner = new Scanner(System.in); // for inputting values for row and column

        String tempPlayer; // empty string for player
        // setting the proper value for player
        if (player == 1) {
            tempPlayer = "X";
        } else{
            tempPlayer = "O";
        }
        // Output message, getting the value for rows and column
    ...
    /* validMove()
            ...
            runs printSymbol is it's valid
